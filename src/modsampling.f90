...
!NetCDF variables
  ...
  real, allocatable, dimension(:,:) ::  ufavl,vfavl,wfavl,thlfavl,thvfavl,qtfavl,qlfavl,nrsampfl,massflxhavl, &  !u and v added
...
!> Performs the actual sampling
  subroutine dosampling
    ...
    use modmpi,    only : slabsum,mpi_integer,comm3d,mpierr,mpi_sum,D_MPI_ALLREDUCE
    use modpois,   only : p
    ...
    do i=2,i1
    do j=2,j1
       u0f (i,j,k) = 0.5*(u0 (i,j,k) + u0  (i+1,j,k)) + cu
       v0f (i,j,k) = 0.5*(v0 (i,j,k) + v0  (i,j+1,k)) + cv
    end do
    end do
    end do
